...
<div class = "row">
<div class = "col-md-6">
```{r mh function log - traceplots, fig.cap="\\label{fig:figs}Figure 3: Trace plots and histograms for the model using log(GDP).", echo = FALSE}
set.seed(100)
...
<div class = "col-md-6">
```{r mh function log - posterior distributions, echo = FALSE}
par(mfrow = c(2, 2))
...
```{r Appendix - gibbs function without protein to check if model estimates and dic are the same when inputting empty variable in MH-sampler, include = FALSE}
gibbssampler <- function(y, x, N, niter = 10000, m00 = 0, t00 = 1000, a0 = 0.001, b0 = 0.001, m01 = 0, t01 = 1000){
  #matrix for each chain to store the sampled values for each parameter
  para.chain1 <- matrix(data = NA, nrow = niter, ncol = 3) 
  para.chain2 <- matrix(data = NA, nrow = niter, ncol = 3)
  
  #starting values
  para.chain1[1,2] <- 0.3
  para.chain2[1,2] <- 0.6
  
  para.chain1[1,3] <- 0.00004
  para.chain2[1,3] <- 0.00006
  
  
  for (i in 2:niter){
    #chain 1
  para.chain1[i-1,1] <- rnorm(n=1, mean = ((sum(y-para.chain1[i-1, 2]*x) / para.chain1[i-1,3]) + (m00/t00)) / ((N / para.chain1[i-1,3]) + (1/t00)), sd = sqrt(1 / ( (N/para.chain1[i-1,3]) + (1/t00)))) 
  
  para.chain1[i,2] <- rnorm(n=1, mean = ( (sum(x*(y - para.chain1[i-1,1]) ) / (para.chain1[i-1,3]) ) + (m01/t01) ) / ( ( (sum(x^2)) / (para.chain1[i-1,3]) ) + (1/t01)), sd =  sqrt(1 / ( (sum(x^2) / para.chain1[i-1,3]) + (1 / t01)) ) )
  
  para.chain1[i,3] <- 1 / rgamma(n = 1, shape = ( (N/2) + a0), rate = ( ( sum(( y - (para.chain1[i-1,1] + para.chain1[i,2]*x) )^2 ) / 2) + b0)) 
  #chain 2
  para.chain2[i-1,1] <- rnorm(n=1, mean = ((sum(y-para.chain2[i-1, 2]*x) / para.chain2[i-1,3]) + (m00/t00)) / ((N / para.chain2[i-1,3]) + (1/t00)), sd = sqrt(1 / ( (N/para.chain2[i-1,3]) + (1/t00)))) 
  
  para.chain2[i,2] <- rnorm(n=1, mean = ( (sum(x*(y - para.chain2[i-1,1]) ) / (para.chain2[i-1,3]) ) + (m01/t01) ) / ( ( (sum(x^2)) / (para.chain2[i-1,3]) ) + (1/t01)), sd =  sqrt(1 / ( (sum(x^2) / para.chain2[i-1,3]) + (1 / t01)) ) )
  
  para.chain2[i,3] <- 1 / rgamma(n = 1, shape = ( (N/2) + a0), rate = ( ( sum(( y - (para.chain2[i-1,1] + para.chain2[i,2]*x) )^2 ) / 2) + b0))
  }
...
par.mean <- round(c(colMeans(burned.chain1), colMeans(burned.chain2), colMeans(burned.pool)), digits = 6)
par.sd <- round(c(sqrt(c(diag(cov(burned.chain1)), diag(cov(burned.chain1)), diag(cov(burned.pool))))), digits = 6)
MCerror <- round( c(par.sd[1:3] / sqrt(niter), par.sd[4:6] / sqrt(niter), par.sd[7:9] / sqrt(2*niter)), digits = 6)
CI.chains <- matrix(round(c(
  apply(X= cbind(burned.chain1, burned.chain2), MARGIN = 2, 
        FUN = quantile, probs = c(0.025, 0.975) )), digits = 6), nrow = 6, ncol = 2, byrow = TRUE)
CI.pool <- matrix(round(c(
  apply(X= burned.pool, MARGIN = 2, 
        FUN = quantile, probs = c(0.025, 0.975) )), digits = 6), nrow = 3, ncol = 2, byrow = TRUE)

samplestats.chain1 <- data.frame(parameter = c("b0", "b1",  "sigma2"), mean = par.mean[1:3], sd =par.sd[1:3], ci2.5 = CI.chains[(1:3),1], ci97.5 = CI.chains[(1:3), 2], MCerror = MCerror[1:3])
colnames(samplestats.chain1) <- c("Chain 1", "Mean", "SD", "2.5%", "97.5%", "MC Error")

samplestats.chain2 <- data.frame(parameter = c("b0", "b1", "sigma2"), mean = par.mean[4:6], sd =par.sd[4:6], ci2.5 = CI.chains[(4:6),1], ci97.5 = CI.chains[(4:6), 2], MCerror = MCerror[4:6])
colnames(samplestats.chain2) <- c("Chain 2", "Mean", "SD", "2.5%", "97.5%", "MC Error")
  
samplestats.pooled <- data.frame(parameter = c("b0", "b1", "sigma2"), mean = par.mean[7:9], sd =par.sd[7:9], ci2.5 = CI.pool[(1:3),1], ci97.5 = CI.pool[(1:3), 2], MCerror = MCerror[7:9])
colnames(samplestats.pooled) <- c("Pooled chain", "Mean", "SD", "2.5%", "97.5%", "MC Error")
  ...
  for(i in 1:nrow(burned.pool)){ #likelihood of each iteration for dbar
    twologlike[i] <- -2*sum(dnorm(y, mean = burned.pool[i,1] + burned.pool[i,2]*x , sd = sqrt(burned.pool[i,3]), log = TRUE))
  }
  dbar = mean(twologlike)
  dhat = 2*sum(dnorm(y, mean = samplestats.pooled$Mean[1] + samplestats.pooled$Mean[2]*x , sd = sqrt(samplestats.pooled$Mean[3]), log = TRUE))
...
#trace plots for convergence
 par(mfrow=c(2,2))
  ts.plot(cbind(burned.chain1[,1], burned.chain2[,1]), col = c("red", "blue"), lwd = 0.5, ylab = "b0", xlab = "iteration")
  ts.plot(cbind(burned.chain1[,2], burned.chain2[,2]), col = c("red", "blue"), lwd = 0.5, ylab = "b1", xlab = "iteration")
  ts.plot(cbind(burned.chain1[,3], burned.chain2[,3]), col = c("red", "blue"), lwd = 0.5, ylab = "sigma^2", xlab = "iteration")
  
  invisible(list(burned.chain1 = burned.chain1, burned.chain2 = burned.chain2, burned.pool = burned.pool, para.chain1 = para.chain1, para.chain2 = para.chain2, samplestats.chain1 = samplestats.chain1, samplestats.chain2 = samplestats.chain2, samplestats.pooled =  samplestats.pooled, dic = dic))
}
gibbs.result <- gibbssampler(dat$mheight, dat$gdp,N = nrow(dat))
gibbs.result$dic
```
